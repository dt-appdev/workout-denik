...
|---|---|---|
| 24. 9. 2026 | F1-03 | Krokovač: v rozdělaném tréninku klepnutí na kg, opakování, čas nebo km neotevře klávesnici, ale dole panel s tlačítky − a + pro všechny hodnoty série pod sebou (podržení mění hodnotu dál). Začíná se od zapsané hodnoty, u prázdného políčka od šedé hodnoty z minula; upravovaná série je v tabulce zvýrazněná a vidět nad panelem. Krok se mění jedním tlačítkem dokola: kg 0,5 → 1 → 1,25 → 2,5 → 5 (výchozí 2,5), čas 1 → 5 → 30 s (výchozí 5), km 0,1 → 0,5 → 1; pamatuje se pro každý cvik (vázaný cvik zvlášť pro každé fitko), jen v telefonu. Opakování po 1. „Napsat“ otevře klávesnici v políčku, „✓ Série hotová“ odškrtne sérii se vším jako ✓ (kontrola čísel, velký skok, rekord, pauza) a panel zavře. Chyba nebo „Opravit“ u velkého skoku otevře krokovač. Nastavení → Zadávání čísel v tréninku: vypínač „Tlačítka +/−“ (výchozí zapnuto, `stepper` v `config/main`). Úprava uloženého tréninku a šablony beze změny. Formát zálohy beze změny. |
