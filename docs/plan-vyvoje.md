...
- [x] **F3-07** Roční heatmapa tréninků (barva podle počtu sérií).
...
|---|---|---|
| 24. 9. 2026 | F3-07 | Statistiky → nový oddíl „Rok v tréninku“ pod grafem Průběh: 12 malých měsíců (3 × 4, týden po–ne), čtvereček = den. Barva podle počtu pracovních sérií za den (víc tréninků v jednom dni se sečte, zahřívací série se nepočítají, cviky na čas a vzdálenost = 1 série), 4 odstíny podle čtvrtin vlastních dní s tréninkem v zobrazeném období, den s nejvíc sériemi vždy nejsytější; legenda ukáže, kolik sérií který odstín znamená. Řídí se filtrem fitek (všechna fitka = barva appky, vybrané fitko = jeho barva), na volbě období nezávisí. Výchozí posledních 12 měsíců, šipky o rok zpět (jen když jsou starší tréninky), klepnutí na název vrátí posledních 12 měsíců; při příchodu do Statistik vždy posledních 12 měsíců. Dnešek orámovaný, budoucí dny se nezobrazují. Dole souhrn (tréninky, série, dny s tréninkem). Klepnutí na den otevře trénink jako v kalendáři (víc věcí = výběr, Zpět se vrátí do výběru). Data ani formát zálohy beze změny. |
