...
- Písmo (F3-11): velikost písma jen přes stupnici v `:root` v `css/app.css` (`--fs-2xs` 11, `--fs-xs` 12, `--fs-sm` 13,
  ...
  „Největší“ na šířku 360 px: rozměry podle textu v `em`, ne pevné px; popisek s ovládáním vedle v `.row.wrap-r`
  s `.grow-a` (ovládání přeskočí na další řádek); pevné lišty přes `min(…, vw)` (`--fs-tabs`); rozměry počítané v JS
  násobit `fontK()` (okraje grafů).
