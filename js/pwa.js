...
navigator.serviceWorker.addEventListener("controllerchange",()=>{
  ...
  if(manual||document.visibilityState==="hidden"||Date.now()-loadedAt<15000)reload();
  else pending=true;
});
const ready=navigator.serviceWorker.register("sw.js",{updateViaCache:"none"}).then(reg=>{
...
window.PWA={
  ...
  async check(){
    const reg=await ready;if(!reg)return "none";
    if(pending){reload();return "new"}
    manual=true; // nová verze nalezená touto kontrolou se načte hned
    try{await reg.update()}catch(e){manual=false;return "offline"}
    if(reg.installing||reg.waiting)return "new";
    manual=false;return "same";
  }
};
})();
