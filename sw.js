  ...
  event.waitUntil(
    (async () => {
      for (const key of await caches.keys()) {
        ...
        if ((key.startsWith(PREFIX) && key !== CACHE) || (!PR && key.startsWith("workout-denik-"))) {
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(
    (async () => {
      ...
      if (res.ok && new URL(req.url).pathname.endsWith("/js/fedb.js")) {
...
async function restLog(txt) {
  ...
  try {
    ...
    if (
      a[0] &&
      txt.startsWith("naplánováno") &&
      a[0].txt.startsWith("naplánováno") &&
      Date.now() - a[0].at < 15000
    ) {
      a.shift();
    }
...
self.addEventListener("message", (event) => {
  ...
  if (old) {
    clearTimeout(old.t);
    old.done();
    restTimers.delete(m.tag);
  }
  if (!m.end) {
    if (old) {
      restLog("zrušeno (konec byl " + new Date(old.end).toLocaleTimeString("cs-CZ") + ")");
    }
    return;
  }
  ...
  event.waitUntil(
    new Promise((done) => {
      ...
      r.t = setTimeout(
        async () => {
          try {
            ...
            const st =
              wins.map((c) => c.visibilityState + (c.focused ? "+aktivní" : "")).join(", ") || "zavřená";
            if (m.always || !seen) {
              ...
              await restLog(
                "konec " +
                  plan +
                  ": oznámení zobrazeno" +
                  (late > 1 ? ", zpoždění " + late + " s" : "") +
                  " (appka: " +
                  st +
                  ")",
              );
            } else {
              await restLog("konec " + plan + ": appka na očích, bez oznámení (appka: " + st + ")");
            }
          } catch (e) {
            await restLog("konec " + plan + ": chyba " + ((e && e.message) || e));
          }
          if (restTimers.get(m.tag) === r) {
            restTimers.delete(m.tag);
          }
          done();
        },
      ...
      if (!old || old.end !== m.end) {
        restLog("naplánováno na " + plan + (m.always ? " (zkouška)" : ""));
      }
    }),
  );
});
